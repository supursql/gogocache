...
func startCacheServer(addr string, addrs []string, gogo *gocache.Group) {
	peers := gocache.NewHTTPPool(addr)
	peers.Set(addrs...)
	gogo.RegisterPeers(peers)
	log.Println("gogocache is running at ", addr)
	log.Fatal(http.ListenAndServe(addr[7:], peers))
}

func startAPIServer(apiAddr string, gogo *gocache.Group) {
	http.Handle("api", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		key := req.URL.Query().Get("key")
		view, err := gogo.Get(key)
		if err != nil {
